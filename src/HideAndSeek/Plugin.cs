...
public sealed class Plugin : BaseUnityPlugin
{
    ...
    {
        HideAndSeekClientData.ApplyHooksAndEvents();
        HideAndSeekLobbyData.ApplyHooksAndEvents();
        HideAndSeekMode.ApplyHooksAndEvents();
        
        RainMeadowCache.Initialize();
    }
    ...
    {
        Assert(!IsInitialized);
        Cfg.Options.Instance = new Cfg.Options();
    ...
    private void On_RainWorld_OnModsInit(On.RainWorld.orig_OnModsInit orig, RainWorld self)
    {
        ...
        try
        {
            ...
            MachineConnector.SetRegisteredOI(Mod.id, Cfg.Options.Instance);
            ApplyHooksAndEvents();
