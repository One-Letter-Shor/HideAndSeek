...
public static class Logger
{
    ...
    /// <summary>
    /// Convenience logging method used to label messages
    /// with a number and separate them from other messages.
    /// </summary>
    /// <inheritdoc cref="Log"/>
    ...
    /// <summary>
    /// Logs a message if at least one flag in <paramref name="logLevels"/>
    /// is enabled in <see cref="Cfg.Options.EnabledLogLevels"/>.
    /// </summary>
    /// <remarks>
    /// <see cref="LogLevel.None"/> messages will always be logged unless
    /// <see cref="Cfg.Options.CfgEnabledLogLevels"/> is also <see cref="LogLevel.None"/>.
    /// If messages are logged before configurables are initialized then the
    /// default <see cref="Cfg.Options.CfgEnabledLogLevels"/> value will be used.
    /// </remarks>
    /// <param name="logLevels">The log level that the data is logged at.</param>
    /// <param name="data">The data that is logged.</param>
    /// <param name="loggingMemberName">Information about the caller.</param>
    /// <param name="loggingLineNumber">Information about the caller.</param>
    /// <param name="loggingFilePath">Information about the caller.</param>
    public static void Log(
        ...
        [CallerFilePath] string loggingFilePath = "")
    {
        // You can log if either are true:
        //  - One flag in logLevels is enabled
        //  - logLevels is None and EnabledLogLevels is not None
        int logLevelsValue = (int)logLevels;
        int enabledLogLevelsValue = (int)Cfg.Options.EnabledLogLevels;
        if ((logLevelsValue & enabledLogLevelsValue) == 0 && (logLevelsValue != 0 && enabledLogLevelsValue != 0)) return; // TODO: Ensure this functions in all cases.
